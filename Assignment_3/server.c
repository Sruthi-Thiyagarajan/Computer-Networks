...
void readFile(struct SubscriberDatabase subDb[]) {
	...
	if(filePointer == NULL)
	{
		printf("\n====ERROR! File not found/Cannot Open File====\n");
		return;
	}
	while(fgets(line, sizeof(line), filePointer) != NULL)
	{
		char * words=NULL;
		words = strtok(line," ");
		subDb[i].subscriberNumber =(unsigned) atol(words);     // ascii to long int - atol
		words = strtok(NULL," ");
		subDb[i].technology = atoi(words);					   // ascii to int - atoi
		words = strtok(NULL," ");
...
// Value -1 - if subscriber not found; 
// Value 2 - If mismatch between subscriber and technology
// Else returns the corresponding paid or not paid status
...
int main(int argc, char**argv){
	...
	socklen_t addr_size;
	sockfd=socket(AF_INET,SOCK_DGRAM,0);     //UDP protocol socket creation
	
    memset(&serverAddr,0,sizeof(serverAddr));  //fills the memory area with the given value - here the given value is 0.
	serverAddr.sin_family = AF_INET;
	serverAddr.sin_addr.s_addr=htonl(INADDR_ANY);
	serverAddr.sin_port=htons(PORT);
	
    bind(sockfd,(struct sockaddr *)&serverAddr,sizeof(serverAddr));   //Binding of socket with socket address
	addr_size = sizeof serverAddr;
	printf("Server Started and is Running\n");
